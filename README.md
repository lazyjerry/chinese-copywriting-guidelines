...
`skills/chinese-copywriting/` 是給 AI 編碼代理（Claude Code、Codex CLI、Copilot CLI、OpenCode、Cursor 等）使用的 skill，把本文的規則做成可自動套用的校對與撰寫流程，並附一支零依賴的檢查腳本。

用 [ai-global](https://github.com/lazyjerry/ai-global) 安裝：
...
也可以把 `skills/chinese-copywriting` 直接複製或 symlink 進各工具的 skills 目錄。
...
完整的安裝、使用、開發與測試說明見 **[SKILL-README.md](SKILL-README.md)**。
