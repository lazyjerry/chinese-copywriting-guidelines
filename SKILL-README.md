...
流程刻意把確認點放在最前面，**開跑之前一次問完四件事**：

1. **校對哪個檔** — 預設是編輯器當前開啟的檔案，會標示「（當前開啟）」放在第一個選項。訊息裡已指定路徑就直接用。定了對象會先確認它是不是產物——校對產物是白做工，下次重跑建置就全部還原。
2. **查完要怎麼處理** — 自動修正可安全處理的項目、全部修正、只出報告不改檔，或先看清單再決定。
3. **要不要擴充英文單位** — 內建 23 個刻意收得窄，可以帶入常見的資訊與電子單位或自己列。擴充進來的一律標低信心度、不自動修，因為單位縮寫跟 `5G`、`4K` 這類產品代號在正則眼裡形狀相同。
4. **有沒有不套規則的區段** — 與外部資料逐字對應的欄位、貼上來的錯誤訊息，這類刻意保持原樣的段落。

已經在 `docs/chinese-copywriting/decisions.md` 記過的不會再問一次。

**絕不自動改檔，也絕不未經確認就開跑。** 未取得同意之前不會執行帶 `--fix` 的指令。

**專案級的裁決會累積。** 決定「英文期刊名保留書名號」「`Nvidia` 不改成 `NVIDIA`」之後，結論寫進 `docs/chinese-copywriting/decisions.md`，下次校對直接沿用不重判。那個目錄整個不進版控，所以裁決只在這台機器上累積——換一台機器是空的，團隊之間也不共享。要全隊沿用同一套裁決，把結論另外寫進有進版控的文案規範。

目標檔是建置產物時，修正落在來源不落在產物：先對產物跑一次 `--fix` 當乾跑確認範圍，把同樣的替換寫回生成腳本或範本，重跑建置，再 `diff` 驗證各份副本一致。
