...
流程刻意把确认点放在最前面，**开跑之前一次问完四件事**：

1. **校对哪个文件** — 默认是编辑器当前打开的文件，会标示「（当前打开）」放在第一个选项。消息里已指定路径就直接用。定了对象会先确认它是不是产物——校对产物是白做工，下次重跑构建就全部还原。
2. **查完要怎么处理** — 自动修正可安全处理的项目、全部修正、只出报告不改文件，或先看清单再决定。
3. **要不要扩充英文单位** — 内建 23 个刻意收得窄，可以带入常见的信息与电子单位或自己列。扩充进来的一律标低信心度、不自动修，因为单位缩写跟 `5G`、`4K` 这类产品代号在正则眼里形状相同。
4. **有没有不套规则的区段** — 与外部数据逐字对应的字段、贴上来的错误信息，这类刻意保持原样的段落。

已经在 `docs/chinese-copywriting/decisions.md` 记过的不会再问一次。

**绝不自动改文件，也绝不未经确认就开跑。** 未取得同意之前不会执行带 `--fix` 的命令。

**项目级的裁决会累积。** 决定「英文期刊名保留书名号」「`Nvidia` 不改成 `NVIDIA`」之后，结论写进 `docs/chinese-copywriting/decisions.md`，下次校对直接沿用不重判。那个目录整个不进版本控制，所以裁决只在这台机器上累积——换一台机器是空的，团队之间也不共享。要全队沿用同一套裁决，把结论另外写进有进版本控制的文案规范。

目标文件是构建产物时，修正落在来源不落在产物：先对产物跑一次 `--fix` 当干跑确认范围，把同样的替换写回生成脚本或模板，重跑构建，再 `diff` 验证各份副本一致。
